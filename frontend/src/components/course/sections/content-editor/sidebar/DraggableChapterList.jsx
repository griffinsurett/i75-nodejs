...
export default function DraggableChapterList({
  ...
  const handleDragOver = (e) => {
    ...
    const items = [
      ...containerRef.current.querySelectorAll("[data-chapter-item]"),
    ];
    ...
    if (closestItem) {
      ...
      if (
        targetChapter &&
        !targetChapter.pendingDeletion &&
        !(targetChapter.chapters || targetChapter).isArchived
      ) {
        setDropPosition({
  ...
  const getDropLinePosition = (index) => {
    ...
    if (
      ...
      dropPosition.sortedIndex === index
    ) {
      return "top";
    }
    if (
      dropPosition.position === "after" &&
      dropPosition.sortedIndex === index + 1
    ) {
      return "bottom";
    }
    return false;
  ...
  return (
    ...
    >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {isArchived && hasScheduledDeletion && (
                        <ChapterDeletionBadge
                          scheduledDeleteAt={
                            chapterData.purgeAfterAt ||
                            chapterData.scheduledDeleteAt
                          }
                          isPending={false}
                        />
                      )}
      ...
      {isDragging &&
        ...
        dropPosition?.sortedIndex === sortedChapters.length && (
