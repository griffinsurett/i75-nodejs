...
const {
  ...
  sections, chapters, tests, questions, options, entries,
  images, videos,
  optionImages, optionVideos,
  questionImages, questionVideos,
  instructors,
 ...
 */
async function insertSnapshot(tx, { entityType, entityId, action, ttlMinutes, payload }) {
  const deleteAfter =
...
async function buildCoursePayload(tx, courseId) {
  const [course] = await tx.select().from(courses).where(eq(courses.courseId, courseId));
...
async function buildSectionPayload(tx, sectionId) {
  const [section] = await tx.select().from(sections).where(eq(sections.sectionId, sectionId));
  if (!section) return null;
  return { section };
}

async function buildChapterPayload(tx, chapterId) {
  const [chapter] = await tx.select().from(chapters).where(eq(chapters.chapterId, chapterId));
...
async function buildEntryPayload(tx, entryId) {
  const [entry] = await tx.select().from(entries).where(eq(entries.entryId, entryId));
  if (!entry) return null;
  return { entry };
}

async function buildImagePayload(tx, imageId) {
  const [image] = await tx.select().from(images).where(eq(images.imageId, imageId));
  if (!image) return null;
  return { image };
}

async function buildVideoPayload(tx, videoId) {
  const [video] = await tx.select().from(videos).where(eq(videos.videoId, videoId));
  if (!video) return null;
  return { video };
}

async function buildInstructorPayload(tx, instructorId) {
  const [inst] = await tx.select().from(instructors).where(eq(instructors.instructorId, instructorId));
  if (!inst) return null;
  return { instructor: inst };
}

async function buildOptionPayload(tx, optionId) {
  const [opt] = await tx.select().from(options).where(eq(options.optionId, optionId));
  if (!opt) return null;

  const imgs = await tx.select().from(optionImages).where(eq(optionImages.optionId, optionId));
  const vids = await tx.select().from(optionVideos).where(eq(optionVideos.optionId, optionId));

  return { option: opt, optionImages: imgs, optionVideos: vids };
}

async function buildQuestionPayload(tx, questionId) {
  const [q] = await tx.select().from(questions).where(eq(questions.questionId, questionId));
  if (!q) return null;

  const qImgs = await tx.select().from(questionImages).where(eq(questionImages.questionId, questionId));
  const qVids = await tx.select().from(questionVideos).where(eq(questionVideos.questionId, questionId));

  const opts = await tx.select().from(options).where(eq(options.questionId, questionId));
  const optIds = opts.map(o => o.optionId);
  const oImgs = optIds.length ? await tx.select().from(optionImages).where(inArray(optionImages.optionId, optIds)) : [];
  const oVids = optIds.length ? await tx.select().from(optionVideos).where(inArray(optionVideos.optionId, optIds)) : [];
...
async function restoreQuestion(tx, payload) {
  const { question, questionImages: qImgs, questionVideos: qVids, options: opts, optionImages: oImgs, optionVideos: oVids } = payload;
