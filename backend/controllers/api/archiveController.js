...
const { insertSnapshot, builders, restorers } = require("../../utils/archiver");
...
const archiveController = {
  ...
  list: async (req, res, next) => {
    try {
      ...
      const where = [];
      if (status && status !== "all") where.push(eq(archiveSnapshots.status, status));
  ...
  get: async (req, res, next) => {
    try {
      ...
      const [row] = await db.select().from(archiveSnapshots).where(eq(archiveSnapshots.archiveId, archiveId));
      if (!row) return res.status(404).json({ success: false, message: "Archive item not found" });
  ...
  moveToArchive: async (req, res, next) => {
    ...
    try {
      ...
      res.status(201).json({ success: true, data: result, message: action === 'delete'
  ...
  restore: async (req, res, next) => {
    try {
      ...
      const [row] = await db.select().from(archiveSnapshots).where(eq(archiveSnapshots.archiveId, archiveId));
      if (!row) return res.status(404).json({ success: false, message: "Archive item not found" });
      if (!restorers[row.entityType]) return res.status(400).json({ success: false, message: `Unsupported entity type: ${row.entityType}` });
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.update(archiveSnapshots)
          .set({ status: "restored", restoredAt: new Date(), deleteAfter: null })
          .where(eq(archiveSnapshots.archiveId, archiveId));

        // Optional: delete snapshot entirely so archive shows only active items
        await tx.delete(archiveSnapshots).where(eq(archiveSnapshots.archiveId, archiveId));
      });
  ...
  cancelTimer: async (req, res, next) => {
    try {
      ...
      await db.update(archiveSnapshots)
        .set({ status: "archived", deleteAfter: null })
        .where(eq(archiveSnapshots.archiveId, archiveId));

      res.json({ success: true, message: "Deletion timer cancelled; item retained in archive." });
    } catch (e) { next(e); }
  },
  ...
  purgeNow: async (req, res, next) => {
    try {
      const { archiveId } = req.params;
      const del = await db.delete(archiveSnapshots).where(eq(archiveSnapshots.archiveId, archiveId)).returning();
      if (del.length === 0) return res.status(404).json({ success: false, message: "Archive item not found" });
